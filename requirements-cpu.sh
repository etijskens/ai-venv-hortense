...
if [[ "${VIRTUAL_ENV}" != "/dodrio/scratch/projects/explor_2022_005/.venv/cpu" ]]
then
    echo "Wrong venv activated: '${VIRTUAL_ENV}', expecting '/dodrio/scratch/projects/explor_2022_005/.venv/cpu'."
...
### FORCE_CUDA=1 pip install ${PROJECT_DIR}/EUROSENSE-R-D/torch-points-kernels -v --no-deps --no-binary :all:
pip install ${PROJECT_DIR}/EUROSENSE-R-D/torch-points-kernels -v --no-deps --no-binary :all: |& tee -a requirements-cpu.sh.log
...
pip install torchvision==0.11.1 -v --no-deps |& tee -a requirements-cpu.sh.log
